...
bool netrule::IsInFreeZone(const Point2d& p) const
{
    if (p.X() < fzminx || p.X() > fzmaxx || p.Y() < fzminy || p.Y() > fzmaxy) return 0;
...
{
    if ((p1.X() > fzmaxx && p2.X() > fzmaxx) || (p1.X() < fzminx && p2.X() < fzminx) ||
        (p1.Y() > fzmaxy && p2.Y() > fzmaxy) || (p1.Y() < fzminy && p2.Y() < fzminy)) {
        return false;
    }
...
int netrule::ConvexFreeZone() const
{
    ...
    for (size_t i = 0; i < n; i++) {
        const bool counterclockwise =
            CCW(transfreezone[i], transfreezone[(i + 1) % n], transfreezone[(i + 2) % n], 1e-7);
        if (!counterclockwise) return 0;
    }
    return 1;
}
