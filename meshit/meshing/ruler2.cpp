                              ...
                              std::vector<uint32_t>& dellines, int tolerance, const MeshingParameters& mp)
{
    ...
    for (size_t i = 0; i < maxlegalline; i++) {
        if (lnearness[i] < MAX_NEARNESS) lnearness_class[lnearness[i]]++;
    }
    int cumm = 0;
    for (size_t j = 0; j < MAX_NEARNESS; j++) {
    ...
    for (size_t ri = 0; ri < rules.size(); ri++) {
        ...
        for (int j = 0; j < 2; j++) {
            pmap[rule->GetLine(0)[j]] = llines[0][j];
            pused[llines[0][j] - 1]++;
        }
            ...
            if (nlok < rule->GetNOldL()) {
                    ...
                    for (int j = 0; j < 2; j++) {
                        ...
                        if (pmap[refpi] != 0) {
                            ...
                            if (rule->CalcPointDist(refpi, lpoints[loclin[j] - 1]) > maxerr ||
                                !legalpoints[loclin[j] - 1] || pused[loclin[j] - 1]) {
                                ok = 0;
                                break;
                            }
                        }
                ...
                while (npok >= 0) {
                    if (npok < static_cast<int>(rule->GetNOldP())) {
                        ...
                        for (size_t i = 0; i < maxlegalline; i++) {
                            if (!lused[i] &&
                                rule->IsLineInFreeZone(lpoints[llines[i].I1() - 1],
