...
namespace meshit {
    ...
    void Element::GetIntegrationPoint(int ip, Point<3> & p, double & weight) const
    {
        ...
        double * pp = NULL;
        switch (typ) {
            case TET: pp = &eltetqp[0][0];
                break;
            case TET10: pp = &eltet10qp[ip - 1][0];
                break;
        }
            ...
            class DenseMatrix & trans) const
    {
        ...
        switch (GetType()) {
            case TET: dshapep = &ipdtet.Get(ip)->dshape;
                break;
    ...
    {
        switch (GetType()) {
            case TET: if (ipdtet.size()) return;
                break;
        ...
        for (int i = 1; i <= GetNIP(); i++) {
            ...
            switch (GetType()) {
                case TET: ipdtet.Elem(i).Reset(ipd);
                    break;
