...
//    1 bei illegal termination due to different reasons
{
    ...
    while (it++ <= par.maxit_linsearch) {
        ...
        if (f - phi0 > mu1 * alphahat * phi1prime + eps0 * fabs(phi0)) {
            ...
            double c =
                (f - phi1 - phi1prime * (alphahat - alpha1)) / ((alphahat - alpha1) * (alphahat - alpha1));

            alphahat = alpha1 - 0.5 * phi1prime / c;

            if (alphahat > alpha2) {
                alphahat = alpha1 +
                           1 / (4 * c) * ((sigma + mu1) * phi0prime - 2 * phi1prime +
            ...
            if (phihatprime < sigma * phi0prime * (1 + eps0)) {
                if (phi1prime < phihatprime) {
                    ...
                    alphaincr = (alphahat - alpha1) * phihatprime / (phi1prime - phihatprime);
                } else {
...
double BFGS_2d(double* x, MinFunction_2d& fun, const OptiParameters& par)
{
    ...
    do {
        ...
        alphahat = 1;
        ifail = lines(x, xneu, p, f, g, fun, par, alphahat, fmin, mu1, sigma, xi1, xi2, tau, tau1, tau2);

        if (ifail == 1) std::cerr << "no success with linesearch" << std::endl;
    ...
    if (f0 < f || (ifail == 1)) {
        MESHIT_LOG_WARNING("fail, f = " << f << " f0 = " << f0 << " diff = " << f0 - f
                           << "  ifail = " << ifail);
        f = f0;
        x[0] = x0[0];
        x[1] = x0[1];
