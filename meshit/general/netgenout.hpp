...
class Procs
{
  const meshit::FlatArray<unsigned> procs;

public:

  Procs ( const meshit::FlatArray<unsigned> & aprocs ) : procs (aprocs) { ; }

  const meshit::FlatArray<unsigned> & GetProcs () const { return procs; }
};
...
public:
  ...
  NetgenOutStream(std::ostream * aout, Imp imp ) :
    ...
    printheader(1)
  { 
    if ( meshit::printmessage_importance >= imp.GetImp() )
      print = true;
    else
      print = false;
  }
  ...
  { 
#ifdef PARALLEL
    if ( procs.GetProcs().Contains(meshit::id) )
      print = true;
    else
      print = false;
#else
