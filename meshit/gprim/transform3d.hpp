...
namespace meshit {
    ...
    class Transformation3d
    {
     ...
     public:
        ...
        explicit Transformation3d(const Point3d** pp);
        /// Unit tet is mapped to tet descibed by pp
        explicit Transformation3d(const Point3d pp[]);
        /// translation
        explicit Transformation3d(const Vec3d& translate);
        /// rotation with ...
    ...
    class Transformation
    {
     ...
     public:
        ...
        /// Unit tet is mapped to tet descibed by pp
        explicit Transformation(const Point<D>* pp);

        /// translation
        explicit Transformation(const Vec<D>& translate)
                : m(0), v(translate)
        {
            for (int i = 0; i < D; i++) {
                m(i, i) = 1;
