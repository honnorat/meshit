#ifndef MESHIT_GEOM2D_HPP
#define MESHIT_GEOM2D_HPP
/**
...
class Point2d
{
 ...
 public:
    ...
    bool operator==(const Point2d& p2) const { return (px == p2.px && py == p2.py); }
    double operator[](size_t i) const { return (i == 0) ? px : (i == 1) ? py : 0.0; }
...
{
    return Point2d(0.5 * (p1.px + p2.px), 0.5 * (p1.py + p2.py));
}

inline Point2d Center(const Point2d& p1, const Point2d& p2, const Point2d& p3)
{
    return Point2d(1.0 / 3.0 * (p1.px + p2.px + p3.px), 1.0 / 3.0 * (p1.py + p2.py + p3.py));
}
...
inline bool CW(const Point2d& p1, const Point2d& p2, const Point2d& p3)
{
    ...
    return (p2.px - p1.px) * (p3.py - p2.py) - (p2.py - p1.py) * (p3.px - p2.px) < 0;
}
...
class Box2d
{
 public:
    ...
    Point2d Center() const { return Point2d(0.5 * (pmin.X() + pmax.X()), 0.5 * (pmin.Y() + pmax.Y())); }

    const Box2d& operator+=(const Box2d& b);

    void SetPoint(const Point2d& p) { pmin = pmax = p; }
    ...
    {
        return std::max(pmax.X() - pmin.X(), pmax.Y() - pmin.Y());
    }
